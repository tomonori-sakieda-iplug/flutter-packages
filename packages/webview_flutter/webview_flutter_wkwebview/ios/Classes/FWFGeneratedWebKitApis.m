...
void FWFWKWebViewHostApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FWFWKWebViewHostApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.WKWebViewHostApi.enableJavaScriptCanOpenWindowsAutomatically"
        binaryMessenger:binaryMessenger
        codec:FWFWKWebViewHostApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(enableJavaScriptCanOpenWindowsAutomatically:isEnabled:error:)], @"FWFWKWebViewHostApi api (%@) doesn't respond to @selector(enableJavaScriptCanOpenWindowsAutomatically:isEnabled:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSNumber *arg_identifier = GetNullableObjectAtIndex(args, 0);
        NSNumber *arg_enabled = GetNullableObjectAtIndex(args, 1);
        FlutterError *error;
        [api enableJavaScriptCanOpenWindowsAutomatically:arg_identifier isEnabled:arg_enabled error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
