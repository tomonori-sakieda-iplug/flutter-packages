...
void FWFWKPreferencesHostApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FWFWKPreferencesHostApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.WKPreferencesHostApi.enableJavaScriptCanOpenWindowsAutomatically"
        binaryMessenger:binaryMessenger
        codec:FWFWKPreferencesHostApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(enableJavaScriptCanOpenWindowsAutomaticallyForPreferencesWithIdentifier:isEnabled:error:)], @"FWFWKPreferencesHostApi api (%@) doesn't respond to @selector(enableJavaScriptCanOpenWindowsAutomaticallyForPreferencesWithIdentifier:isEnabled:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSNumber *arg_identifier = GetNullableObjectAtIndex(args, 0);
        NSNumber *arg_enabled = GetNullableObjectAtIndex(args, 1);
        FlutterError *error;
        [api enableJavaScriptCanOpenWindowsAutomaticallyForPreferencesWithIdentifier:arg_identifier isEnabled:arg_enabled error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
